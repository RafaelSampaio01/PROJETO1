...
class Writer():
    ...
    def __init__(self, device, font, verbose=True):
        ...
        if verbose:
            print('Orientation: {} Reversal: {}'.format(
                'horiz' if font.hmap() else 'vert', font.reverse()
            ))
    ...
    def printstring(self, string, invert=False):
        for char in string:
            self._printchar(char, invert=invert)

    # Invert: desenha "texto apagado" em fundo aceso (estilo seleção)
    def _printchar(self, char, invert=False):
        ...
        # Fundo do caractere quando invertido
        if invert:
            self.device.fill_rect(Writer.text_col, Writer.text_row, char_width, char_height, 1)
